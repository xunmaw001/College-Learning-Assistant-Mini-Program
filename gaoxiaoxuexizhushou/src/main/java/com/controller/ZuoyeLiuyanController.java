    ...
    */
    ...
    public R save(@RequestBody ZuoyeLiuyanEntity zuoyeLiuyan, HttpServletRequest request){
        ...
        if(false)
            ...
            zuoyeLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
...
//            zuoyeLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<ZuoyeLiuyanEntity> queryWrapper = new EntityWrapper<ZuoyeLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        ZuoyeLiuyanEntity zuoyeLiuyanEntity = zuoyeLiuyanService.selectOne(queryWrapper);
        zuoyeLiuyan.setUpdateTime(new Date());
        if(zuoyeLiuyanEntity==null){
    ...
    */
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<ZuoyeLiuyanEntity> zuoyeLiuyanList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            ZuoyeLiuyanEntity zuoyeLiuyanEntity = new ZuoyeLiuyanEntity();
//                            zuoyeLiuyanEntity.setZuoyeId(Integer.valueOf(data.get(0)));   //作业 要改的
//                            zuoyeLiuyanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            zuoyeLiuyanEntity.setZuoyeLiuyanText(data.get(0));                    //评论内容 要改的
//                            zuoyeLiuyanEntity.setInsertTime(date);//时间
//                            zuoyeLiuyanEntity.setReplyText(data.get(0));                    //回复内容 要改的
//                            zuoyeLiuyanEntity.setUpdateTime(sdf.parse(data.get(0)));          //回复时间 要改的
//                            zuoyeLiuyanEntity.setCreateTime(date);//时间
                            zuoyeLiuyanList.add(zuoyeLiuyanEntity);
                        ...
                        //查询是否重复
                        zuoyeLiuyanService.insertBatch(zuoyeLiuyanList);
                        return R.ok();
                    }
                }
            }
