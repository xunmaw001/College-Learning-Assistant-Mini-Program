		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<!-- 名字-->
					<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.zuoyeName}}
					</view>
				</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.zuoyeContent"></rich-text>
				</view>
			</view>
		...
		<view class="bottom-content cu-bar bg-white tabbar border shop">
			...
			<view style="text-align: right;">
				...
				<button @tap="onSubmit" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur"
                            :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3],borderColor:btnColor[3]}">上传作业</button>
			</view>
		</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('zuoye', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.zuoyePhoto ? this.detail.zuoyePhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('zuoyeLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					zuoyeId: this.id
				});
			...
			async onSubmit(){
				this.$utils.jump(`../huida/add-or-update?zuoyeId=${this.id}`)
			},
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../zuoyeLiuyan/add-or-update?zuoyeId=${this.id}`)
			},
		}
	}
</script>
