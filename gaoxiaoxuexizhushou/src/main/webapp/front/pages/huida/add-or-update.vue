	...
	<view class="content">
		<form class="app-update-pv">
			<view
				...
				class="cu-form-group" @tap="huifuFileTap">
				...
				<view class="right-input" style="padding:0;textAlign:left">
					<image
						...
						class="avator" v-if="ruleForm.huifuFile" :src="ruleForm.huifuFile" mode="aspectFill">
					</image>
					<text>
						{{ruleForm.huifuFile?"已上传":"未上传"}}
					</text>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			...
			/*下拉框*/
			let huidaTypesParams = {
				page: 1,
				limit: 100,
				dicCode: 'huida_types',
			}
			let huidaTypes = await this.$api.page(`dictionary`, huidaTypesParams);
			this.huidaTypesOptions = huidaTypes.data.list
			...
			if (options.id) {
				...
				// 获取信息
				let res = await this.$api.info(`huida`, this.ruleForm.id);
		...
		methods: {
			// 下拉变化
			huidaTypesChange(e) {
				this.huidaTypesIndex = e.target.value
				this.ruleForm.huidaValue = this.huidaTypesOptions[this.huidaTypesIndex].indexName
				this.ruleForm.huidaTypes = this.huidaTypesOptions[this.huidaTypesIndex].codeIndex
			},
			...
			huifuFileTap() {
				...
				this.$api.upload(function(res) {
					_this.ruleForm.huifuFile = _this.$base.url + 'upload/' + res.file;
					_this.$forceUpdate();
					_this.$nextTick(() => {
						// _this.styleChange()
					})
			...
			async onSubmitTap() {
				if (this.ruleForm.id) {
					await this.$api.update(`huida`, this.ruleForm);
				} else {
					await this.$api.save(`huida`, this.ruleForm);
				}
