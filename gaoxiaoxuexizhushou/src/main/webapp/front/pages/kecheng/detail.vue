...
<script>
	export default {
		...
		methods: {
			...
			　　var shareObj = {
			　　　　title: "一起来学习这个课程吧!!",    // 默认是小程序的名称(可以写slogan等)
			　　　　path: '/pages/kecheng/detail?id=' + this.id,    // 默认是当前页面，必须是以‘/'开头的完整路径
			　　　　imageUrl: '',   //自定义图片路径，可以是本地文件路径、代码包文件路径或者网络图片路径，支持PNG及JPG，不传入 imageUrl 则使用默认截图。显示图片长宽比是 5:4
			　　　　success: function(res){
			　　　　　　// 转发成功之后的回调
			　　　　　　if(res.errMsg == 'shareAppMessage:ok'){
							_this.$tuils.msg("分享成功")
			　　　　　　}
			　　　　},
			　　　　fail: function(){
			　　　　　　// 转发失败之后的回调
			　　　　　　if(res.errMsg == 'shareAppMessage:fail cancel'){
							_this.$tuils.msg("已取消分享")
			
			　　　　　　}else if(res.errMsg == 'shareAppMessage:fail'){
							_this.$tuils.msg("分享失败")
			　　　　　　}
			　　　　}
			　　};
			},
